...
var require_debugSession = __commonJS({
  "node_modules/@vscode/debugadapter/lib/debugSession.js"(exports) {
    ...
    var DebugSession3 = class extends protocol_1.ProtocolServer {
      ...
      sendErrorResponse(response, codeOrMessage, format, variables, dest = ErrorDestination.User) {
        ...
        response.message = DebugSession3.formatPII(msg.format, true, msg.variables);
        if (!response.body) {
          response.body = {};
        }
      ...
      convertClientPathToDebugger(clientPath) {
        if (this._clientPathsAreURIs !== this._debuggerPathsAreURIs) {
          if (this._clientPathsAreURIs) {
            return DebugSession3.uri2path(clientPath);
          } else {
            return DebugSession3.path2uri(clientPath);
          }
        }
        return clientPath;
      }
      convertDebuggerPathToClient(debuggerPath) {
        if (this._debuggerPathsAreURIs !== this._clientPathsAreURIs) {
          if (this._debuggerPathsAreURIs) {
            return DebugSession3.uri2path(debuggerPath);
          } else {
            return DebugSession3.path2uri(debuggerPath);
          }
        }
        return debuggerPath;
      }
      ...
      static formatPII(format, excludePII, args) {
        return format.replace(DebugSession3._formatPIIRegexp, function(match, paramName) {
    ...
    exports.DebugSession = DebugSession3;
    DebugSession3._formatPIIRegexp = /{([^}]+)}/g;
  }
});
...
`, logger_1.LogLevel.Error);
            try {
              await fs.promises.mkdir(path3.dirname(options.logFilePath), { recursive: true });
...
var require_logger = __commonJS({
  "node_modules/@vscode/debugadapter/lib/logger.js"(exports) {
    ...
    var LogLevel2;
    (function(LogLevel3) {
      LogLevel3[LogLevel3["Verbose"] = 0] = "Verbose";
      LogLevel3[LogLevel3["Log"] = 1] = "Log";
      LogLevel3[LogLevel3["Warn"] = 2] = "Warn";
      LogLevel3[LogLevel3["Error"] = 3] = "Error";
      LogLevel3[LogLevel3["Stop"] = 4] = "Stop";
    })(LogLevel2 = exports.LogLevel || (exports.LogLevel = {}));
    var Logger = class {
      constructor() {
        this._pendingLogQ = [];
      }
      log(msg, level = LogLevel2.Log) {
        msg = msg + "\n";
        this._write(msg, level);
      }
      verbose(msg) {
        this.log(msg, LogLevel2.Verbose);
      }
      warn(msg) {
        this.log(msg, LogLevel2.Warn);
      }
      error(msg) {
        this.log(msg, LogLevel2.Error);
      }
      dispose() {
      ...
      _write(msg, level = LogLevel2.Log) {
        msg = msg + "";
    ...
    var LogOutputEvent = class extends debugSession_1.OutputEvent {
      constructor(msg, level) {
        const category = level === LogLevel2.Error ? "stderr" : level === LogLevel2.Warn ? "console" : "stdout";
        super(msg, category);
      }
    };
  ...
  "node_modules/@vscode/debugadapter/lib/loggingDebugSession.js"(exports) {
    ...
    var LoggingDebugSession = class extends debugSession_1.DebugSession {
      constructor(obsolete_logFilePath, obsolete_debuggerLinesAndColumnsStartAt1, obsolete_isServer) {
        ...
        this.on("error", (event) => {
          logger4.error(event.body);
        });
      }
      start(inStream, outStream) {
        super.start(inStream, outStream);
        logger4.init((e) => this.sendEvent(e), this.obsolete_logFilePath, this._isServer);
      }
      sendEvent(event) {
        if (!(event instanceof Logger.LogOutputEvent)) {
          ...
          logger4.verbose(`To client: ${JSON.stringify(objectToLog)}`);
        }
        super.sendEvent(event);
      }
      sendRequest(command, args, timeout, cb) {
        logger4.verbose(`To client: ${JSON.stringify(command)}(${JSON.stringify(args)}), timeout: ${timeout}`);
        super.sendRequest(command, args, timeout, cb);
      }
      sendResponse(response) {
        logger4.verbose(`To client: ${JSON.stringify(response)}`);
        super.sendResponse(response);
      }
      dispatchRequest(request) {
        logger4.verbose(`From client: ${request.command}(${JSON.stringify(request.arguments)})`);
...
// src/dbgmits/mi_output.ts
var require_mi_output = __commonJS({
  "src/dbgmits/mi_output.ts"(exports, module2) {
    var MIOutput;
    ((MIOutput2) => {
      let RecordType2;
      ((RecordType3) => {
        RecordType3[RecordType3["Done"] = 0] = "Done";
        RecordType3[RecordType3["Running"] = 1] = "Running";
        RecordType3[RecordType3["Connected"] = 2] = "Connected";
        RecordType3[RecordType3["Error"] = 3] = "Error";
        RecordType3[RecordType3["Exit"] = 4] = "Exit";
        RecordType3[RecordType3["AsyncExec"] = 5] = "AsyncExec";
        RecordType3[RecordType3["AsyncStatus"] = 6] = "AsyncStatus";
        RecordType3[RecordType3["AsyncNotify"] = 7] = "AsyncNotify";
        RecordType3[RecordType3["DebuggerConsoleOutput"] = 8] = "DebuggerConsoleOutput";
        RecordType3[RecordType3["TargetOutput"] = 9] = "TargetOutput";
        RecordType3[RecordType3["DebuggerLogOutput"] = 10] = "DebuggerLogOutput";
      })(RecordType2 = MIOutput2.RecordType || (MIOutput2.RecordType = {}));
      function getAsyncRecordType(char) {
        switch (char) {
          case "*":
            return 5 /* AsyncExec */;
          case "+":
            return 6 /* AsyncStatus */;
          case "=":
            return 7 /* AsyncNotify */;
        }
      }
      MIOutput2.getAsyncRecordType = getAsyncRecordType;
      ...
      MIOutput2.createObjFromResultList = createObjFromResultList;
    })(MIOutput || (MIOutput = {}));
    module2.exports = MIOutput;
  }
});

// src/dbgmits/mi_output_parser.js
var require_mi_output_parser = __commonJS({
  "src/dbgmits/mi_output_parser.js"(exports, module2) {
    "use strict";
    ...
    function peg$parse(input, options) {
      ...
      var peg$FAILED = {}, peg$startRuleFunctions = { start: peg$parsestart }, peg$startRuleFunction = peg$parsestart, peg$c0 = "^", peg$c1 = peg$literalExpectation("^", false), peg$c2 = function(t, resultType, results) {
        return {
          token: t,
          recordType: resultType,
          data: mioutput.createObjFromResultList(results)
        };
      }, peg$c3 = /^[*+=]/, peg$c4 = peg$classExpectation(["*", "+", "="], false, false), peg$c5 = function(t, at, ac, results) {
        return {
          token: t,
          recordType: mioutput.getAsyncRecordType(at),
          data: [ac, mioutput.createObjFromResultList(results)]
        };
      }, peg$c6 = "done", peg$c7 = peg$literalExpectation("done", false), peg$c8 = function() {
        return mioutput.RecordType.Done;
      }, peg$c9 = "running", peg$c10 = peg$literalExpectation("running", false), peg$c11 = function() {
        return mioutput.RecordType.Running;
      }, peg$c12 = "connected", peg$c13 = peg$literalExpectation("connected", false), peg$c14 = function() {
        return mioutput.RecordType.Connected;
      }, peg$c15 = "error", peg$c16 = peg$literalExpectation("error", false), peg$c17 = function() {
        return mioutput.RecordType.Error;
      }, peg$c18 = "exit", peg$c19 = peg$literalExpectation("exit", false), peg$c20 = function() {
        return mioutput.RecordType.Exit;
      }, peg$c21 = ",", peg$c22 = peg$literalExpectation(",", false), peg$c23 = function(r) {
        return r;
      ...
      }, peg$c25 = "=", peg$c26 = peg$literalExpectation("=", false), peg$c27 = function(n, v, rest) {
        return {
          name: n,
        ...
        return values;
      }, peg$c30 = peg$otherExpectation("variable-identifier"), peg$c31 = function() {
        return text();
      }, peg$c32 = /^[a-z]/i, peg$c33 = peg$classExpectation([["a", "z"]], false, true), peg$c34 = /^[\-_]/, peg$c35 = peg$classExpectation(["-", "_"], false, false), peg$c36 = "{}", peg$c37 = peg$literalExpectation("{}", false), peg$c38 = function() {
        return {};
      }, peg$c39 = "{", peg$c40 = peg$literalExpectation("{", false), peg$c41 = "}", peg$c42 = peg$literalExpectation("}", false), peg$c43 = function(results) {
        return results;
      }, peg$c44 = "[]", peg$c45 = peg$literalExpectation("[]", false), peg$c46 = function() {
        return [];
      }, peg$c47 = "[", peg$c48 = peg$literalExpectation("[", false), peg$c49 = "]", peg$c50 = peg$literalExpectation("]", false), peg$c51 = function(values) {
        return values;
      }, peg$c52 = "~", peg$c53 = peg$literalExpectation("~", false), peg$c54 = function(streamText) {
        return {
          recordType: mioutput.RecordType.DebuggerConsoleOutput,
          data: streamText
        };
      }, peg$c55 = "@", peg$c56 = peg$literalExpectation("@", false), peg$c57 = function(streamText) {
        return {
          recordType: mioutput.RecordType.TargetOutput,
          data: streamText
        };
      }, peg$c58 = "&", peg$c59 = peg$literalExpectation("&", false), peg$c60 = function(streamText) {
        return {
          recordType: mioutput.RecordType.DebuggerLogOutput,
          data: streamText
        };
      }, peg$c61 = peg$otherExpectation("double-quoted-string"), peg$c62 = '"', peg$c63 = peg$literalExpectation('"', false), peg$c64 = function(chars) {
        return chars.join("");
      }, peg$c65 = "'", peg$c66 = peg$literalExpectation("'", false), peg$c67 = "\\", peg$c68 = peg$literalExpectation("\\", false), peg$c69 = "n", peg$c70 = peg$literalExpectation("n", false), peg$c71 = function() {
        return "\n";
      }, peg$c72 = "r", peg$c73 = peg$literalExpectation("r", false), peg$c74 = function() {
        return "\r";
      }, peg$c75 = "t", peg$c76 = peg$literalExpectation("t", false), peg$c77 = function() {
        return "	";
      }, peg$c78 = "", peg$c79 = function() {
        return "\\";
      }, peg$c80 = peg$anyExpectation(), peg$c81 = function(char) {
        return char;
      }, peg$c82 = /^[0-9]/, peg$c83 = peg$classExpectation([["0", "9"]], false, false), peg$c84 = function(digits) {
        return digits.join("");
      ...
      var mioutput = require_mi_output();
...
// src/extension.ts
var import_debugadapter3 = __toESM(require_main());
...
var TerminalEscape = class {
  constructor() {
  }
  static apply({ msg, style }) {
    return style.join("") + msg + "\x1B[0m" /* Reset */;
  }
};

// src/dbgmits/extractors.ts
var _extractFullNameFunction = null;
...
function extractFrameInfo(data) {
  return {
    ...
    line: data.line ? parseInt(data.line, 10) : void 0
  };
}
function extractBreakpointLocationInfo(data) {
  return {
    id: data["number"],
    isEnabled: data.enabled !== void 0 ? data.enabled === "y" : void 0,
    address: data.addr,
    func: data.func,
...
function extractBreakpointInfo(data) {
  ...
  return {
    ...
    isTemp: breakpoint.disp !== void 0 ? breakpoint.disp === "del" : void 0,
    isEnabled: breakpoint.enabled !== void 0 ? breakpoint.enabled === "y" : void 0,
    locations,
    ...
    isInstalled: breakpoint.installed !== void 0 ? breakpoint.installed === "y" : void 0,
...
function extractWatchChildren(data) {
  var extractWatchChild = (data2) => {
    return {
      id: data2.name,
      childCount: parseInt(data2.numchild, 10),
      value: data2.value,
      expressionType: data2["type"],
      threadId: parseInt(data2["thread-id"], 10),
      hasMoreChildren: data2.has_more !== void 0 && data2.has_more !== "0",
      isDynamic: data2.dynamic === "1",
      displayHint: data2.displayhint,
      expression: data2.exp,
      isFrozen: data2.frozen === "1"
    };
  };
  if (data === void 0 || Array.isArray(data)) {
    return [];
  } else if (Array.isArray(data.child)) {
    return data.child.map((child) => {
      return extractWatchChild(child);
    });
  } else {
...
function extractAsmInstructions(data) {
  return data.map((asmInstruction) => {
    return {
...
function extractAsmBySourceLine(data) {
  let extractSrcAsmLine = (data2) => {
    return {
      line: parseInt(data2.line, 10),
      file: data2.file,
      fullname: getFullname(data2.fullname),
      instructions: extractAsmInstructions(data2.line_asm_insn)
    };
  };
  if (data === void 0 || Array.isArray(data)) {
    return [];
...
function extractThreadInfo(data) {
  return {
    ...
    isStopped: data.state === "stopped" ? true : data.state === "running" ? false : void 0,
    processorCore: data.core,
    details: data.details
  };
}

// src/dbgmits/events.ts
var EVENT_SESSION_STARTED = "sess_started";
var EVENT_THREAD_GROUP_ADDED = "thdgrpadd";
var EVENT_THREAD_GROUP_REMOVED = "thdgrprem";
var EVENT_THREAD_GROUP_STARTED = "thdgrpstart";
var EVENT_THREAD_GROUP_EXITED = "thdgrpexit";
var EVENT_THREAD_CREATED = "thdcreate";
var EVENT_THREAD_EXITED = "thdexit";
var EVENT_THREAD_SELECTED = "thdselect";
var EVENT_LIB_LOADED = "libload";
var EVENT_LIB_UNLOADED = "libunload";
var EVENT_DBG_CONSOLE_OUTPUT = "conout";
var EVENT_TARGET_OUTPUT = "targetout";
var EVENT_DBG_LOG_OUTPUT = "dbgout";
var EVENT_TARGET_RUNNING = "targetrun";
var EVENT_TARGET_STOPPED = "targetstop";
var EVENT_BREAKPOINT_HIT = "brkpthit";
var EVENT_STEP_FINISHED = "endstep";
var EVENT_FUNCTION_FINISHED = "endfunc";
var EVENT_SIGNAL_RECEIVED = "signal";
var EVENT_EXCEPTION_RECEIVED = "exception";
var EVENT_BREAKPOINT_MODIFIED = "breakpoint-modified";
function createEventsForExecNotification(notification, data) {
  switch (notification) {
    ...
    case "stopped":
      ...
      let events2 = [{ name: EVENT_TARGET_STOPPED, data: stopEvent }];
      switch (stopEvent.reason) {
        case 0 /* BreakpointHit */:
          let breakpointHitEvent = {
          ...
          events2.push({ name: EVENT_BREAKPOINT_HIT, data: breakpointHitEvent });
          break;
        case 1 /* EndSteppingRange */:
          let stepFinishedEvent = {
            reason: stopEvent.reason,
            threadId: stopEvent.threadId,
            stoppedThreads: stopEvent.stoppedThreads,
            processorCore: stopEvent.processorCore,
          ...
          events2.push({ name: EVENT_STEP_FINISHED, data: stepFinishedEvent });
          break;
        case 2 /* FunctionFinished */:
          let stepOutEvent = {
            reason: stopEvent.reason,
            threadId: stopEvent.threadId,
            stoppedThreads: stopEvent.stoppedThreads,
          ...
          events2.push({ name: EVENT_FUNCTION_FINISHED, data: stepOutEvent });
          break;
        case 4 /* SignalReceived */:
          let signalEvent = {
            reason: stopEvent.reason,
            threadId: stopEvent.threadId,
            stoppedThreads: stopEvent.stoppedThreads,
          ...
          events2.push({ name: EVENT_SIGNAL_RECEIVED, data: signalEvent });
          break;
        case 5 /* ExceptionReceived */:
          let exceptionEvent = {
            reason: stopEvent.reason,
            threadId: stopEvent.threadId,
            stoppedThreads: stopEvent.stoppedThreads,
          ...
          events2.push({ name: EVENT_EXCEPTION_RECEIVED, data: exceptionEvent });
          break;
      }
      return events2;
    default:
...
function createEventForAsyncNotification(notification, data) {
  switch (notification) {
    case "thread-group-added":
      return { name: EVENT_THREAD_GROUP_ADDED, data };
    case "thread-group-removed":
      return { name: EVENT_THREAD_GROUP_REMOVED, data };
    case "thread-group-started":
      return { name: EVENT_THREAD_GROUP_STARTED, data };
    case "thread-group-exited":
      let groupExitedEvent = {
        id: data.id,
...
var targetStopReasonMap = (/* @__PURE__ */ new Map()).set("breakpoint-hit", 0 /* BreakpointHit */).set("end-stepping-range", 1 /* EndSteppingRange */).set("function-finished", 2 /* FunctionFinished */).set("exited-normally", 3 /* ExitedNormally */).set("exited-signalled", 7 /* ExitedSignalled */).set("exited", 8 /* Exited */).set("signal-received", 4 /* SignalReceived */).set("exception-received", 5 /* ExceptionReceived */);
function parseTargetStopReason(reasonString) {
  var reasonCode = targetStopReasonMap.get(reasonString);
  if (reasonCode !== void 0) {
    return reasonCode;
  }
  return 6 /* Unrecognized */;
}
...
// src/dbgmits/errors.ts
var CommandFailedError = class {
...
var MalformedResponseError = class {
...
// src/dbgmits/debug_session.ts
var readline = __toESM(require("readline"));
var events = __toESM(require("events"));
var parser = __toESM(require_mi_output_parser());
var import_mi_output = __toESM(require_mi_output());
var import_debugadapter = __toESM(require_main());
var DebugCommand = class {
  constructor(cmd, token, done, isNative = false) {
    this.token = token;
    this.text = cmd;
    this.done = done;
...
var DebugSession = class extends events.EventEmitter {
  constructor() {
    super();
  ...
  end(notifyDebugger = true) {
    return new Promise((resolve, reject) => {
      var cleanup = (err, data) => {
  ...
  canEmitFunctionFinishedNotification() {
    return false;
  }
  emitExecNotification(name, data) {
    let events2 = createEventsForExecNotification(name, data);
    events2.forEach((event) => {
      this.emit(event.name, event.data);
    });
  }
  emitAsyncNotification(name, data) {
    let event = createEventForAsyncNotification(name, data);
    if (event) {
      this.emit(event.name, event.data);
    } else {
      if (this.logger) {
        this.logger.warn(JSON.stringify({ name, data }) + " Unhandled notification.");
      }
    }
  }
  parseDebbugerOutput(line) {
    ...
    try {
      ...
      this.emit(EVENT_DBG_CONSOLE_OUTPUT, line);
      return;
    }
    switch (result.recordType) {
      case import_mi_output.RecordType.Done:
      case import_mi_output.RecordType.Running:
      case import_mi_output.RecordType.Connected:
      case import_mi_output.RecordType.Exit:
      case import_mi_output.RecordType.Error:
        var cmd = this.cmdQueue.shift();
        cmdQueuePopped = true;
        if (cmd.done) {
          if (result.recordType === import_mi_output.RecordType.Error) {
            cmd.done(new CommandFailedError(result.data.msg, cmd.text, result.data.code, cmd.token), null);
          } else {
            cmd.done(null, result.data);
          }
        }
        break;
      case import_mi_output.RecordType.AsyncExec:
        this.emitExecNotification(result.data[0], result.data[1]);
        break;
      case import_mi_output.RecordType.AsyncNotify:
        this.emitAsyncNotification(result.data[0], result.data[1]);
        break;
      case import_mi_output.RecordType.DebuggerConsoleOutput:
        this.emit(EVENT_DBG_CONSOLE_OUTPUT, result.data);
        break;
      case import_mi_output.RecordType.TargetOutput:
        this.emit(EVENT_TARGET_OUTPUT, result.data);
        break;
      case import_mi_output.RecordType.DebuggerLogOutput:
        this.emit(EVENT_DBG_LOG_OUTPUT, result.data);
        break;
    }
  ...
  executeCommand(command, token) {
    return new Promise((resolve, reject) => {
      this.enqueueCommand(new DebugCommand(command, token, (err, data) => {
        err ? reject(err) : resolve();
      }));
    });
  }
  getCommandOutput(command, token, transformOutput) {
    return new Promise((resolve, reject) => {
  ...
  getNativeCommandOutput(command, token, transformOutput) {
    return new Promise((resolve, reject) => {
      this.enqueueCommand(new DebugCommand(command, token, (err, data) => {
        if (err) {
          reject(err);
  ...
  addBreakpoint(location, options) {
    ...
    return this.getCommandOutput(cmd + " " + location, null, extractBreakpointInfo);
  }
  removeBreakpoint(breakId) {
    return this.executeCommand("break-delete " + breakId);
  }
  removeBreakpoints(breakIds) {
    return this.executeCommand("break-delete " + breakIds.join(" "));
  }
  enableBreakpoint(breakId) {
    return this.executeCommand("break-enable " + breakId);
  }
  enableBreakpoints(breakIds) {
    return this.executeCommand("break-enable " + breakIds.join(" "));
  }
  disableBreakpoint(breakId) {
    return this.executeCommand("break-disable " + breakId);
  }
  disableBreakpoints(breakIds) {
    return this.executeCommand("break-disable " + breakIds.join(" "));
  }
  ignoreBreakpoint(breakId, ignoreCount) {
    return this.getCommandOutput(`break-after ${breakId} ${ignoreCount}`, null, extractBreakpointInfo);
  }
  setBreakpointCondition(breakId, condition) {
    return this.executeCommand(`break-condition ${breakId} ${condition}`);
  }
  setInferiorArguments(args) {
  ...
  resumeAllThreads(reverse = false) {
  ...
  getStackFrames(options) {
    ...
    if (options) {
      ...
      if (options.lowFrame !== void 0 && options.highFrame !== void 0) {
        fullCmd = fullCmd + ` ${options.lowFrame} ${options.highFrame}`;
      } else if (options.lowFrame !== void 0) {
        fullCmd = fullCmd + ` ${options.lowFrame} ${options.lowFrame}`;
      } else if (options.highFrame !== void 0) {
    ...
    return this.getCommandOutput(fullCmd, null, (output) => {
      ...
      if (Array.isArray(data)) {
        return data.map((frame) => {
          return extractStackFrameInfo(frame);
        });
      } else {
  ...
  getStackFrameArgs(detail, options) {
    ...
    if (options) {
      if (options.lowFrame !== void 0 && options.highFrame !== void 0) {
        fullCmd = fullCmd + ` ${options.lowFrame} ${options.highFrame}`;
      } else if (options.lowFrame !== void 0 && options.highFrame === void 0) {
  ...
  getStackFrameVariables(detail, options) {
    let fullCmd = "stack-list-variables";
    if (options) {
      if (options.threadId !== void 0) {
        fullCmd = fullCmd + " --thread " + options.threadId;
      }
    ...
    return this.getCommandOutput(fullCmd, null, (output) => {
      let args = [];
      let locals = [];
      output.variables.forEach((varInfo) => {
        if (varInfo.arg === "1") {
  ...
  updateWatch(id, detail) {
    ...
    fullCmd = fullCmd + " " + id;
    return this.getCommandOutput(fullCmd, null, (output) => {
      return output.changelist.map((data) => {
        return {
          id: data.name,
          childCount: data.new_num_children ? parseInt(data.new_num_children, 10) : void 0,
          value: data.value,
  ...
  getWatchChildren(id, options) {
    var fullCmd = "var-list-children";
    if (options && options.detail !== void 0) {
      fullCmd = fullCmd + " " + options.detail;
    }
    fullCmd = fullCmd + " " + id;
    if (options && options.from !== void 0 && options.to !== void 0) {
  ...
  setWatchValueFormat(id, formatSpec) {
    var fullCmd = `var-set-format ${id} ` + watchFormatSpecToStringMap.get(formatSpec);
    return this.getCommandOutput(fullCmd, null, (output) => {
      if (output.value) {
        return output.value;
      } else {
        return output.changelist[0].value;
      }
    });
  }
  getWatchValue(id, formatSpec) {
    var fullCmd = "var-evaluate-expression";
    if (formatSpec !== void 0) {
  ...
  setWatchValue(id, expression) {
    return this.getCommandOutput(`var-assign ${id} "${expression}"`, null, (output) => {
      return output.value;
    });
  }
  getWatchAttributes(id) {
    var cmd = "var-show-attributes " + id;
    return this.getCommandOutput(cmd, null, (output) => {
      if (output.status) {
        return [stringToWatchAttributeMap.get(output.status)];
      } else if (output.attr) {
        if (Array.isArray(output.attr)) {
          return output.attr.map((attr) => {
      ...
      throw new MalformedResponseError('Expected to find "status" or "attr", found neither.', output, cmd);
    });
  }
  ...
  readMemory(address, numBytesToRead, options) {
  ...
  getRegisterValues(formatSpec, options) {
    ...
    return this.getCommandOutput(fullCmd, null, (output) => {
      ...
      var registerMap = /* @__PURE__ */ new Map();
      if (registers) {
  ...
  disassembleAddressRange(start, end, showOpcodes) {
  ...
  disassembleAddressRangeByLine(start, end, showOpcodes) {
  ...
  disassembleFile(filename, line, options) {
    var fullCmd = `data-disassemble -f ${filename} -l ${line}`;
    if (options && options.maxInstructions !== void 0) {
  ...
  disassembleFileByLine(filename, line, options) {
    var fullCmd = `data-disassemble -f ${filename} -l ${line}`;
    if (options && options.maxInstructions !== void 0) {
      fullCmd = fullCmd + " -n " + options.maxInstructions;
    }
  ...
  complete(input) {
    ...
    return this.getCommandOutput(fullCmd, null, (output) => {
      ...
      import_debugadapter.logger.error(e);
    });
    ;
  }
  interpreterExec(command) {
  ...
  targetAttach(pid) {
    var fullCmd = `target-attach ${pid}`;
    return this.executeCommand(fullCmd, null);
  }
  execNativeCommand(cmd) {
    return this.getNativeCommandOutput(cmd, null, (output) => {
      return output;
    }).catch((e) => {
      import_debugadapter.logger.error(e);
      throw e;
  ...
  environmentCd(path3) {
    var fullCmd = `environment-cd ${path3}`;
    return this.executeCommand(fullCmd, null);
  }
  targetFilePut(hostfile, targetfile) {
    var fullCmd = `target-file-put ${hostfile} ${targetfile}`;
    return this.executeCommand(fullCmd, null);
  }
};
...
var watchFormatSpecToStringMap = (/* @__PURE__ */ new Map()).set(0 /* Binary */, "binary").set(1 /* Decimal */, "decimal").set(2 /* Hexadecimal */, "hexadecimal").set(3 /* Octal */, "octal").set(4 /* Default */, "natural");
var stringToWatchAttributeMap = (/* @__PURE__ */ new Map()).set("editable", 0 /* Editable */).set("noneditable", 1 /* NonEditable */);
var registerValueFormatSpecToCodeMap = (/* @__PURE__ */ new Map()).set(0 /* Binary */, "t").set(1 /* Decimal */, "d").set(2 /* Hexadecimal */, "x").set(3 /* Octal */, "o").set(4 /* Raw */, "r").set(5 /* Default */, "N");
...
var DDBSessionImpl = class extends DebugSession {
  ...
  async startDDB(args) {
    ...
    let check_version = (out) => {
      if (out.startsWith("GNU gdb")) {
        ...
        this.removeListener(EVENT_DBG_CONSOLE_OUTPUT, check_version);
      }
    };
    ...
    if (!currentTerminal) {
      currentTerminal = vscode2.window.createTerminal("DDB");
    }
    currentTerminal.show(true);
    let pid = await currentTerminal.processId;
    console.log("Current terminal pid:", pid);
    var tty = "/dev/pts/0";
    (0, import_child_process.exec)(`ps h -o tty -p ${pid}|tail -n 1`, (error, stdout, stderr) => {
      if (!error) {
        tty = "/dev/" + stdout;
      }
    ...
    this.debuggerProcess.on("error", (error) => {
      ...
      this.emit(EVENT_TARGET_STOPPED, { reason: 8 /* Exited */ });
    });
    ...
    return new Promise((resolve, reject) => {
      try {
        if (this.debuggerProcess) {
          process2.kill(this.debuggerProcess.pid, "SIGINT");
        }
      } catch (error) {
        reject();
      }
  ...
  kill() {
    return new Promise((resolve, reject) => {
      try {
        if (this.debuggerProcess) {
          process2.kill(this.debuggerProcess.pid);
        }
        resolve();
      } catch (error) {
        reject();
...
var EVENT_CONFIG_DOWN = "configdown";
var DistDebug = class extends import_debugadapter2.DebugSession {
  constructor() {
    super(true);
    this._variableHandles = new import_debugadapter2.Handles();
    this._locals = { frame: null, vars: [], watch: [] };
    ...
    this.varUpperCase = false;
    this._addressesInHex = true;
  }
  printToDebugConsole(msg, itype = 0 /* info */) {
    let style = ["\x1B[34m" /* Blue */];
    switch (itype) {
      case 1 /* error */:
        style = ["\x1B[31m" /* Red */];
        break;
    ...
    this.sendEvent(new import_debugadapter2.OutputEvent(TerminalEscape.apply({ msg, style })));
  }
  waitForConfingureDone() {
    return new Promise((resolve, reject) => {
  ...
  initDebugSession() {
    ...
    this.ddbServer.on(EVENT_SIGNAL_RECEIVED, (event) => {
      import_debugadapter2.logger.log(event.reason.toString());
    });
    this.ddbServer.on(EVENT_DBG_CONSOLE_OUTPUT, (out) => {
      this.printToDebugConsole(out);
    });
    this.ddbServer.on(EVENT_TARGET_RUNNING, (out) => {
      this._isRunning = true;
      import_debugadapter2.logger.log(out);
    });
    this.ddbServer.on(EVENT_TARGET_STOPPED, (e) => {
      import_debugadapter2.logger.log("stoped:" + e.reason.toString());
      this._isRunning = false;
      ...
      switch (e.reason) {
        ...
        case 5 /* ExceptionReceived */:
          break;
        case 7 /* ExitedSignalled */:
        case 8 /* Exited */:
        case 3 /* ExitedNormally */:
    ...
    this.ddbServer.on(EVENT_FUNCTION_FINISHED, (e) => {
      this.sendEvent(new import_debugadapter2.StoppedEvent("function breakpoint", e.threadId));
    });
    this.ddbServer.on(EVENT_SIGNAL_RECEIVED, (e) => {
      import_debugadapter2.logger.log("signal_receive:" + e.signalCode);
      let event = new import_debugadapter2.StoppedEvent("signal", e.threadId, e.signalMeaning);
      event.body["text"] = e.signalMeaning;
      event.body["description"] = e.signalMeaning;
      this.sendEvent(event);
    });
    this.ddbServer.on(EVENT_EXCEPTION_RECEIVED, (e) => {
      this.sendEvent(new import_debugadapter2.StoppedEvent("exception", e.threadId, e.exception));
    });
  }
  initializeRequest(response, args) {
  ...
  continueRequest(response, args) {
    console.log("--- Resuming all threads (Resume execution -exec-run) ---", args);
    this.ddbServer.resumeAllThreads();
    this.sendResponse(response);
  }
  nextRequest(response, args, request) {
    console.log("--- Next (Step Over Function Calls -exec-next -- right arrow) ---");
  ...
  stepBackRequest(response, args, request) {
    ...
    this.ddbServer.stepOverLine({ reverse: true }).catch((e) => {
  ...
  async launchRequest(response, _args) {
    ...
    let ret = await this.ddbServer.setExecutableFile(args.program).catch((e) => {
    ...
    if (args.remote?.enabled) {
      if (!args.remote.address) {
        vscode3.window.showErrorMessage("Invalid remote addr.");
      }
      let mode = args.remote.mode === void 0 ? "remote" : args.remote.mode;
      if (mode === "remote") {
        let result = await this.ddbServer.connectToRemoteTargetEx(args.remote.address).catch((e) => {
          this.printToDebugConsole(e.message, 1 /* error */);
          vscode3.window.showErrorMessage(e.message);
          return 1;
        });
        if (result > 0) {
          this.sendEvent(new import_debugadapter2.TerminatedEvent(false));
        ...
        let result = await this.ddbServer.connectToRemoteTargetEx(args.remote.address, mode).catch((e) => {
          this.printToDebugConsole(e.message, 1 /* error */);
          vscode3.window.showErrorMessage(e.message);
          return 1;
        });
        ...
        if (args.remote.transfer) {
          ...
          for (const trans of args.remote.transfer) {
            ...
            const startEvent = new import_debugadapter2.ProgressStartEvent(id, `upload ${trans.from}`);
...
`, 3 /* info2 */);
            ...
            await this.ddbServer.targetFilePut(trans.from, trans.to).catch((e) => {
              vscode3.window.showErrorMessage(e.message);
              this.sendEvent(new import_debugadapter2.ProgressEndEvent(id, e.message));
            }).then(() => {
              this.printToDebugConsole(`file uploaded : ${trans.from}
`, 3 /* info2 */);
              this.sendEvent(new import_debugadapter2.ProgressEndEvent(id, endMessage));
            });
          }
        }
        ...
        vscode3.window.showErrorMessage("Invalid remote mode.");
        this.sendEvent(new import_debugadapter2.TerminatedEvent(false));
        return;
      }
    }
    if (this.language == "auto") {
      let checklang = (out) => {
        if (out.indexOf("language") > 0) {
          ...
          this.ddbServer.off(EVENT_DBG_CONSOLE_OUTPUT, checklang);
        }
      };
      this.ddbServer.on(EVENT_DBG_CONSOLE_OUTPUT, checklang);
      await this.ddbServer.execNativeCommand("show language");
    }
    await this.ddbServer.startAllInferiors(args.stopAtEntry).catch((e) => {
      this.printToDebugConsole(e.message, 1 /* error */);
      vscode3.window.showErrorMessage("Failed to start the debugger." + e.message);
      this.sendEvent(new import_debugadapter2.TerminatedEvent(false));
    });
  ...
  async setBreakPointsRequest(response, args) {
    ...
    srcpath = path2.normalize(srcpath);
    ...
    const actualBreakpoints = await Promise.all(clientLines.map(async (l) => {
      ...
      const bp = new import_debugadapter2.Breakpoint(false, this.convertDebuggerLineToClient(l.line));
  ...
  async stackTraceRequest(response, args) {
    ...
    response.body = {
      stackFrames: frames.map((f) => {
        return new import_debugadapter2.StackFrame(f.level, f.func, f.filename ? new import_debugadapter2.Source(f.filename, f.fullname) : null, this.convertDebuggerLineToClient(f.line));
      }),
  ...
  async variablesRequest(response, args, request) {
    ...
    if (id === "locals::") {
      ...
      this._locals.watch = [];
      const vals = await this.ddbServer.getStackFrameVariables(2 /* Simple */, {
      ...
      if (id.startsWith("**FLIST**")) {
        ...
        let childs = await this.ddbServer.getWatchChildren(id, { detail: 1 /* All */ }).catch((e) => {
          return [];
        });
  ...
  decodeString(value, expressionType) {
  ...
  customRequest(command, response, args) {
    if (command === "toggleFormatting") {
      ...
      if (this._useInvalidatedEvent) {
        this.sendEvent(new import_debugadapter2.InvalidatedEvent(["variables"]));
      }
      this.sendResponse(response);
    } else {
