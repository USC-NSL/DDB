...
class RestoreContext(gdb.Command):
	def __init__(self):
		gdb.Command.__init__(self, "rcontext", gdb.COMMAND_STACK, gdb.COMPLETE_NONE)
				...
				for symbol in cur_frame.block():
					if symbol.is_argument or symbol.is_variable:
						if symbol.name == "msg":
							...
							byte_array_type = gdb.lookup_type("unsigned char").array(length - 1).pointer()
							data_ptr_casted = data_ptr.cast(byte_array_type)
							# Now, you can read the bytes
							byte_array = bytearray()
							for i in range(length):
								byte_array.append(int(data_ptr_casted.dereference()[i]))
							...
							parent_rsp = int.from_bytes(metadata[0:8], byteorder='little')
							parent_rip = int.from_bytes(metadata[-8:], byteorder='little')

							print(f"parent_rsp: {parent_rsp}")
							print(f"parent_rip {parent_rip}")
						if symbol.name=="c":
							sc=symbol.value(cur_frame).dereference()
							tcp_conn_type = gdb.lookup_type('net.TCPConn').pointer()
							tcp_addr_type = gdb.lookup_type('net.TCPAddr').pointer()
							fd=sc["c"]["data"].cast(tcp_conn_type).dereference()["conn"]["fd"].dereference()
							parent_addr=fd["raddr"]["data"].cast(tcp_addr_type).dereference()
							port = int(parent_addr['Port'])
							ip_address=[int(b) for b in SliceValue(parent_addr['IP'])]
							
		return {"metadata":{"parentRIP":parent_rip,"parentRSP":parent_rsp,"parentAddr":ip_address,"parent_port":port}}
