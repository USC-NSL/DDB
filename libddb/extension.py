...
class GetLockState(gdb.Command):
    ...
    def __init__(self):
        super(GetLockState, self).__init__("get-lock-state",
                                                  gdb.COMMAND_DATA,
                                                  gdb.COMPLETE_SYMBOL)
    ...
    def invoke(self, arg, from_tty):
        ...
        if ddb_shared is not None:
            ...
            # print(f"Found {thread_count} threads and {lock_count} locks")
            # print(f"Mutex debug level: {mutex_debug_level}")
            # print("\nThread data:", thread_data)
            # print("\nLock data:", lock_data)
        # print(ddb_shared["ddb_thread_infos"].)
