...
//     for (i = 0; i < LDB_MAX_NTHREAD; i++) {
//       struct thread *th = &threads[i];
//       if (ldb_shared->ddb_thread_infos[i].id == 0)
//         continue;

//       // initialize per-thread state
//       th->thread_id = ldb_shared->ddb_thread_infos[i].id;
//       uint64_t *tmp = (uint64_t *)ldb_shared->ddb_thread_infos[i].fsbase;
//       th->tls = (struct tls_shared_region *)(tmp - 43);
//       th->stack_base = (uint64_t *)ldb_shared->ddb_thread_infos[i].stackbase;
